...
/// <summary>
/// Marks a circle on an image and reports the marked frame. The circle is stated as
/// the pixel at its centre and a radius, and one whose edge reaches past the frame is
/// drawn up to it. A radius of zero is the single pixel the centre names.
/// <para>
...
/// </para>
/// </summary>
public sealed class DrawCircleExecutor : INodeExecutor
{
    ...
    public DrawCircleExecutor(ILeaseLedger ledger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!DrawingStyle.TryReadCircle(request, out Point centre, out int radius, out NodeDiagnostic? circleFailure))
        {
            return Task.FromResult(NodeExecutionResult.Failure(circleFailure!));
        }
        ...
        try
        {
            ...
            Cv2.Circle(output, centre, radius, colour, thickness);
