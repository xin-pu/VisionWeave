...
| 5 | Existing #22 / `feat/22-editor-shell-theme-foundation` | Implement only the WPF shell and semantic theme resources: dark-amber tokens, layout regions, status/snackbar presentation, keyboard focus, and design-time sample data. No editable Nodify canvas yet. Delivered as the named regions of section 6.5, `Themes/Tokens.xaml` plus `Themes/Shell.xaml`, `ShellStatus`, and `SnackbarNotificationPresenter` in section 6.5 of the detailed design. | 1 | App launches to the documented regions; no business rule lives in code-behind; theme-resource tests or inspection evidence cover key states and contrast; App tests remain green. |
| 6 | `feat/nodify-canvas-projection` | Project the document to generic nodes, ports, and connectors. Implement pan, zoom, selection, add/delete/connect intents, rejection rollback, and undo/redo synchronization. Delivered as `CanvasProjection` plus the gesture layer of `CanvasViewModel` over the Nodify editor, with `RemoveNodesCommand` making a delete one undo unit; see section 6.6 of the detailed design. | 2, 3, 5 | UI smoke test proves add, valid/invalid connect, selection, delete, undo, redo, and visual rollback after a rejected command. |
| 7 | `feat/inspector-document-ux` | Add generic parameter editors, diagnostics panel, dirty/save/open/recovery prompts, unsupported-document view, and node-catalogue search. Delivered as `ParameterEditorViewModel` (one field per declared kind), `InspectorViewModel`, `ShellPromptViewModel`, the document commands over `EditorSession.Read` and `EditorSession.Adopt`, and the read-only presentation; see section 6.7 of the detailed design. | 4, 6 | UI smoke test edits a parameter, shows a targeted diagnostic, saves/reopens, and blocks edits to an unsupported document. |
| 8 | `feat/first-runnable-image-workflow` | Deliver file-image input, one transform (resize or blur), image output, managed preview, run/cancel/status, and working-directory policy. Delivered as the `Image Source` and `Save Image` definitions with their executors, the path rule of [ADR-0012](../adr/0012-file-access-and-the-working-directory.md), `RunWorkflowCommand` with its Cancel gesture and the run outcome it reports, `PreviewViewModel` behind the preview seam, and the preview region it is drawn in; see sections 5.4, 6.5, 6.8, 8, and 10 of the detailed design. | 4, 7 | A golden workflow opens, validates, runs on a real image, displays a managed preview, writes output, cancels safely, and leaves zero native-frame leases. |
